...
pred_test = MagicMock(
    return_value=(
        np.array([[3, 1, 2], [4, 2, 7], [5, 1, 2]]),
        pd.DataFrame([[4, 2, 2], [3, 2, 1], [4, 5, 4]], index=None),
    )
)
...
def test_create_df_check_not_empty(get_y_folder):
    for file in get_y_folder:
        assert (
            pd.read_csv(c.BASE_DIR + "/" + c.path_to_y + "/" + file, header=None).empty
            == False
        )
...
def test_create_df_check_shape(get_y_folder):
    for file in get_y_folder:
        assert pd.read_csv(
            c.BASE_DIR + "/" + c.path_to_y + "/" + file, header=None
        ).shape == (1, len(c.labels))
